...
import device.AXI4Flash

class StandAloneFlash (
  useTL: Boolean = false,
  baseAddress: BigInt,
  addrWidth: Int,
  dataWidth: Int = 64,
  ...
  private val flash = LazyModule(new AXI4Flash(Seq(addressSet)))
  flash.node := TLToAXI4() := xbar

  class StandAloneFlashImp(outer: StandAloneFlash)(implicit p: Parameters) extends StandAloneDeviceImp(outer)

  override lazy val module = new StandAloneFlashImp(this)
